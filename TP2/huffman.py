...
def tuple_to_huffcode(hufftuple):
    ret=''
    n = hufftuple[1]
    for i in range(hufftuple[0]):
        ret+=str(bin(n&1))[2:]
        n=n>>1
    return ret[::-1]
...
def decode(encoded_data, table):
    decoded_data = np.empty(0, dtype=int)
    print("decoding")
    ...
    for byte in encoded_data:
        ...
        for m in masks:
            ...
            if (size, buffer) in lookup:
                ...
                decoded_data = np.append(decoded_data, symbol)
...
if __name__=='__main__':
    ...
    encoded = np.empty(0, dtype=str)
    for i in arr:
        encoded = np.append(encoded, tuple_to_huffcode(table[i]))
